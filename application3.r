...
#  NULL HYPOTHESIS: H_0: beta_4 = beta_5 = 0                                   								                              
...
ppp1   <- data$ppp/1000
IQ2    <- data$IQ^2
mulc   <- data$mulculmano/100
d      <- ifelse(mulc>=0.5,1,0)
trade2 <- data$trade^2
...
mX    <- cbind(rep(1,n),IQ,IQ2,life,tradeL,ppp1,int1,int2) 
mZ    <- cbind(rep(1,n),IQ)

# Matrices of covariates (restricted) 
mXr   <- cbind(rep(1,n),IQ,IQ2,ppp1,int1,int2)
mZ    <- cbind(rep(1,n),IQ) 

# Covariate indices under H_0  
indice_X_H0 <- c(1,2,3,6,7,8) 
indice_Z_H0 <- c(9,10) 
...
# Initial values 
cfr      <- coef(lm(y_star~mX[,2]+mX[,3]+mX[,6]+mX[,7]+mX[,8]))
etaolsr  <- mXr%*%cfr
...
# Model estimation 
estima_null <- betareg(y~mX[,2]+mX[,3]+mX[,6]+mX[,7]+mX[,8] | mZ[,2], start=c(cfr,cf1r), 
...
# Pseudo R-squared based on likelihood
estima_int <- betareg(y~1 | 1, link="logit", link.phi="sqrt", data = data) 
pseudoR2LR <- 1.0 - (exp(estima_int$loglik)/exp(estima_full$loglik))^(2/n)
...
med      <- rbind(round(c(AIC, AICc, BIC,pseudoR2,pseudoR2LR),4))
colnames(med)<-c("AIC","AICc","BIC","PseudoR2", "PseudoR2LR")
rownames(med)<-c("")
