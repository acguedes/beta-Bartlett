...
mXr   <- cbind(rep(1,n),IQ,IQ2,life,ppp1,int1,int2)
mZ    <- cbind(rep(1,n),IQ) 

# Covariate indices under H_0  
indice_X_H0 <- c(1,2,3,4,6,7,8) 
indice_Z_H0 <- c(9,10) 
...
# Initial values 
cfr      <- coef(lm(y_star~mX[,2]+mX[,3]+mX[,4]+mX[,6]+mX[,7]+mX[,8]))
etaolsr  <- mXr%*%cfr
...
# Model estimation 
estima_null <- betareg(y~mX[,2]+mX[,3]+mX[,4]+mX[,6]+mX[,7]+mX[,8] | mZ[,2], start=c(cfr,cf1r), 
